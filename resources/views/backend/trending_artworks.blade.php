@extends('layouts.app', [
    'class' => '',
    'elementActive' => 'trending_artwork'
])
                    ...
                    <div class="card shadow">
                        <div class="card-header border-0">
                            <div class="row align-items-center">
                                <div class="col-8">
                                    <h3 class="mb-0">{{ __('Featured Artwork') }}</h3>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <table id="datatable" class="table table-striped table-bordered table-responsive-sm" cellspacing="0" width="100%">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-right">
                                                        <a href="{{url('/admin/delete_artwork')}}/{{$artwork->id}}/trending_artwork/{{$artwork->user_id}}" class="btn btn-danger btn-link btn-sm delete_artwork" title="Delete Artwork"><i class="fa fa-trash"></i></a>
                                                        <a href="{{url('/admin/change_artwork_status')}}/{{$artwork->id}}/{{$artwork->is_publised}}/trending_artwork/{{$artwork->user_id}}" class="btn btn-danger btn-link btn-sm change_artwork_status" title="@if($artwork->is_publised == 'publish') Click to Un-Publish @else Click to Publish @endif"><i class="fa fa-power-off"></i></a>
                                                        <a href="{{url('/admin/change_top_status')}}/{{$artwork->id}}/{{$artwork->top}}/trending_artwork/{{$artwork->user_id}}" class="btn btn-danger btn-link btn-sm change_top_status" title="@if($artwork->top == 'yes') Remove From Top @else Add to Top @endif">@if($artwork->top == 'yes') <i class="fa fa-minus-square"></i> @else <i class="fa fa-plus-square"></i> @endif</a>
                                                        <a href="{{url('/admin/change_trending_status')}}/{{$artwork->id}}/{{$artwork->trending}}/trending_artwork/{{$artwork->user_id}}" class="btn btn-danger btn-link btn-sm change_trending_status" title="@if($artwork->trending == 'yes') Remove From Featured @else Add to Featured @endif">@if($artwork->trending == 'yes') <i class="fa fa-minus-square"></i> @else <i class="fa fa-plus-square"></i> @endif</a>
                                                    </td>
                                                </tr>
                                                @endforeach
                                            </tbody>
